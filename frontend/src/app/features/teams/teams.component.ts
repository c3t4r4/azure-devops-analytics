...
interface TeamWithCtx extends AzureTeam { orgName: string; }
            ...
            <div class="rounded-lg border border-border bg-card p-5 hover:shadow-md transition-shadow">
              <div class="flex items-start gap-3">
                ...
                <div class="min-w-0">
                  ...
                  @if (team.description) {
                    <p class="text-xs text-muted-foreground mt-0.5 line-clamp-2">{{ team.description }}</p>
                  }
                  <div class="mt-2 flex items-center gap-2">
                    <span class="text-xs bg-muted px-2 py-0.5 rounded-full text-muted-foreground">{{ team.projectName }}</span>
    ...
    this.orgService.getAll().subscribe(orgs => {
      ...
      forkJoin(active.map(org => this.azureService.getProjects(org.name).pipe(catchError(() => of([]))))).subscribe(ppa => {
        ...
        ppa.forEach((projects, i) => projects.slice(0, 5).forEach(p => {
          calls.push(this.azureService.getTeams(active[i].name, p.id).pipe(catchError(() => of([]))));
        ...
        forkJoin(calls).subscribe(results => {
          ...
          results.forEach((teams, i) => teams.forEach((t: AzureTeam) => all.push({ ...t, projectName: meta[i].projectName, orgName: meta[i].orgName })));
          this.teams.set(all);
