  ...
  template: `
    <div class="space-y-8 animate-fade-in">
      <div>
        ...
        <p class="text-muted-foreground">Ranking de usuários e prioridade de projetos por atividade</p>
      </div>
      ...
      <section>
        ...
        @if (rankingLoading()) {
              ...
              <table class="w-full text-sm">
                <thead>
                  <tr class="border-b border-border bg-muted/50">
                    ...
                    <th class="px-4 py-3 text-left text-xs font-medium text-muted-foreground uppercase">Usuário</th>
                    <th class="px-4 py-3 text-center text-xs font-medium text-muted-foreground uppercase">Commits</th>
                    <th class="px-4 py-3 text-center text-xs font-medium text-muted-foreground uppercase">Work Items</th>
                    <th class="px-4 py-3 text-center text-xs font-medium text-muted-foreground uppercase">Pull Requests</th>
                    <th class="px-4 py-3 text-center text-xs font-medium text-muted-foreground uppercase">Builds</th>
                    <th class="px-4 py-3 text-center text-xs font-medium text-muted-foreground uppercase">Score</th>
                  </tr>
                </thead>
                <tbody class="divide-y divide-border">
                  @for (user of ranking()!.rankings; track user.displayName + user.userId; let i = $index) {
                    <tr class="hover:bg-muted/30 transition-colors" [class.bg-primary/5]="i < 3">
                      <td class="px-4 py-3 text-center">
                        @if (i === 0) { <span class="text-lg">🥇</span> }
                        @else if (i === 1) { <span class="text-lg">🥈</span> }
                        @else if (i === 2) { <span class="text-lg">🥉</span> }
                        @else { <span class="text-muted-foreground text-xs">{{ i + 1 }}</span> }
                      </td>
                      <td class="px-4 py-3">
                        <div class="font-medium text-foreground">{{ user.displayName }}</div>
                        @if (user.uniqueName && user.uniqueName !== user.displayName && !user.uniqueName.includes('@')) {
                          <div class="text-xs text-muted-foreground">{{ user.uniqueName }}</div>
                        }
                      </td>
                      <td class="px-4 py-3 text-center">
                        <span class="font-mono text-sm" [class.text-primary]="user.commitCount > 0" [class.text-muted-foreground]="user.commitCount === 0">
                          {{ user.commitCount }}
                        </span>
                      </td>
                      <td class="px-4 py-3 text-center">
                        <span class="font-mono text-sm" [class.text-foreground]="user.workItemsChanged > 0" [class.text-muted-foreground]="user.workItemsChanged === 0">
                          {{ user.workItemsChanged }}
                        </span>
                      </td>
                      <td class="px-4 py-3 text-center">
                        <span class="font-mono text-sm" [class.text-success]="user.pullRequestsCreated > 0" [class.text-muted-foreground]="user.pullRequestsCreated === 0">
                          {{ user.pullRequestsCreated }}
                        </span>
                      </td>
                      <td class="px-4 py-3 text-center text-muted-foreground font-mono text-xs">{{ user.buildsTriggered }}</td>
                      <td class="px-4 py-3 text-center">
                        <span class="font-bold text-foreground bg-primary/10 px-2 py-0.5 rounded-full text-xs">
                          {{ user.totalScore }}
                        </span>
                      </td>
                    </tr>
          ...
          <p class="text-xs text-muted-foreground mt-2">
            Score = Commits×3 + Work Items×2 + Pull Requests×4 + Builds×1 &nbsp;·&nbsp; Período: {{ ranking()?.period }}
          </p>
        } @else if (!rankingLoading()) {
          <div class="rounded-lg border border-dashed border-border bg-card p-8 text-center">
            <p class="text-sm text-muted-foreground">Nenhuma atividade encontrada no período de {{ rankingDays() }} dias.</p>
            <p class="text-xs text-muted-foreground mt-1">O PAT precisa de permissões de leitura em Code, Build e Work Items.</p>
          </div>
        }
      </section>
      ...
      <section>
        ...
        @if (priorityLoading()) {
            ...
            <div class="overflow-x-auto">
              <table class="w-full text-sm">
                <thead>
                  <tr class="border-b border-border bg-muted/50">
                    <th class="px-4 py-3 text-left text-xs font-medium text-muted-foreground uppercase w-12">#</th>
                    <th class="px-4 py-3 text-left text-xs font-medium text-muted-foreground uppercase">Projeto</th>
                    <th class="px-4 py-3 text-center text-xs font-medium text-muted-foreground uppercase">Work Items</th>
                    <th class="px-4 py-3 text-center text-xs font-medium text-muted-foreground uppercase">Commits</th>
                    <th class="px-4 py-3 text-center text-xs font-medium text-muted-foreground uppercase">Builds</th>
                    <th class="px-4 py-3 text-center text-xs font-medium text-muted-foreground uppercase">Score</th>
                    <th class="px-4 py-3 text-left text-xs font-medium text-muted-foreground uppercase">Atividade</th>
                  </tr>
                </thead>
                <tbody class="divide-y divide-border">
                  ...
                  @for (proj of priority()!.projects; track proj.projectId; let i = $index) {
                    <tr class="hover:bg-muted/30 transition-colors">
                      <td class="px-4 py-3 text-center text-muted-foreground text-xs">{{ i + 1 }}</td>
                      <td class="px-4 py-3 font-medium text-foreground">{{ proj.projectName }}</td>
                      <td class="px-4 py-3 text-center text-muted-foreground font-mono text-xs">{{ proj.workItemsChanged }}</td>
                      <td class="px-4 py-3 text-center text-muted-foreground font-mono text-xs">{{ proj.commits }}</td>
                      <td class="px-4 py-3 text-center text-muted-foreground font-mono text-xs">{{ proj.builds }}</td>
                      <td class="px-4 py-3 text-center">
                        <span class="font-bold text-xs bg-primary/10 text-foreground px-2 py-0.5 rounded-full">{{ proj.activityScore }}</span>
                      </td>
          ...
          <p class="text-xs text-muted-foreground mt-2">
            Score = Work Items×1 + Commits×2 + Builds×3 &nbsp;·&nbsp; Período: {{ priority()?.period }}
          </p>
        } @else if (!priorityLoading()) {
          <div class="rounded-lg border border-dashed border-border bg-card p-8 text-center">
            <p class="text-sm text-muted-foreground">Nenhuma atividade de projeto encontrada nos últimos {{ priorityDays() }} dias.</p>
          </div>
        }
      </section>
    </div>
