...
interface SprintWithContext extends AzureSprint {
  orgName: string; projectId: string; projectName: string; teamName: string;
}
    ...
    <div class="space-y-6 animate-fade-in">
      ...
      @if (loading()) {
        ...
        <div class="rounded-lg border border-dashed border-border bg-card p-12 text-center">
          <p class="text-sm text-muted-foreground">Nenhum sprint encontrado. Verifique as permissões do PAT (Work > Read).</p>
        </div>
      } @else {
        @for (group of groupedSprints(); track group.projectKey) {
          <div class="rounded-lg border border-border bg-card overflow-hidden">
            <div class="px-5 py-3 border-b border-border bg-muted/30 flex items-center gap-2">
              ...
              <span class="ml-auto text-xs text-muted-foreground">{{ group.sprints.length }} sprints</span>
            </div>
            <div class="divide-y divide-border">
              @for (sprint of group.sprints; track sprint.id + sprint.orgName) {
                <div class="px-5 py-4 flex items-center justify-between hover:bg-muted/20 transition-colors">
                    ...
                    <div>
                      ...
                      <p class="text-xs text-muted-foreground mt-0.5">
                        ...
                        @if (sprint.workItemCount !== undefined) {
                          &nbsp;·&nbsp;
                          <span class="font-medium text-foreground">{{ sprint.workItemCount }} itens</span>
                          <span class="text-muted-foreground">(</span>
                          <span class="text-success">{{ sprint.completedCount ?? 0 }} concluídos</span>
                          <span class="text-muted-foreground"> / </span>
                          <span class="text-warning">{{ sprint.incompleteCount ?? 0 }} pendentes</span>
                          <span class="text-muted-foreground">)</span>
                        }
                      </p>
                    </div>
            ...
            <div>
              ...
              <p class="text-sm text-muted-foreground">
                ...
                @if (boardSprint()!.startDate && boardSprint()!.finishDate) {
                  · {{ formatDate(boardSprint()!.startDate) }} → {{ formatDate(boardSprint()!.finishDate) }}
                }
              </p>
            </div>
            <button
...
export class SprintsComponent implements OnInit {
  ...
  groupedSprints = computed(() => {
    const groups = new Map<string, { projectKey: string; projectName: string; orgName: string; sprints: SprintWithContext[] }>();
    for (const sprint of this.sprints()) {
      const key = `${sprint.orgName}/${sprint.projectName}`;
      if (!groups.has(key)) groups.set(key, { projectKey: key, projectName: sprint.projectName, orgName: sprint.orgName, sprints: [] });
  ...
  ngOnInit() {
    this.orgService.getAll().subscribe(orgs => {
      ...
      forkJoin(active.map(org =>
        ...
        if (!teamCalls.length) { this.loading.set(false); return; }

        forkJoin(teamCalls.map(tc =>
          this.azureService.getTeams(tc.orgName, tc.projectId).pipe(catchError(() => of([])))
        )).subscribe(teamsPerProject => {
          const sprintCalls: Array<{ orgName: string; projectId: string; projectName: string; team: AzureTeam }> = [];
          teamsPerProject.forEach((teams, i) =>
            teams.slice(0, 2).forEach(t =>
              sprintCalls.push({ ...teamCalls[i], team: t })
            )
          );

          if (!sprintCalls.length) { this.loading.set(false); return; }

          forkJoin(sprintCalls.map(sc =>
            this.azureService.getSprints(sc.orgName, sc.projectId, sc.team.id, true).pipe(catchError(() => of([])))
          )).subscribe(sprintsPerTeam => {
            const all: SprintWithContext[] = [];
            sprintsPerTeam.forEach((sp, i) =>
              sp.forEach(s => all.push({
                ...s,
  ...
  openSprintBoard(sprint: SprintWithContext) {
    ...
    this.azureService.getSprintWorkItems(sprint.orgName, sprint.projectId, sprint.teamId!, sprint.id)
      .pipe(catchError(() => of([])))
      .subscribe(items => {
  ...
  sprintVariant(status?: string): 'success' | 'default' | 'secondary' {
    return ({ current: 'success', inProgress: 'success', future: 'default' } as Record<string, 'success' | 'default' | 'secondary'>)[status ?? ''] ?? 'secondary';
  }
  sprintLabel(status?: string): string {
    return ({ current: 'Atual', inProgress: 'Em andamento', future: 'Futuro', past: 'Encerrado' } as Record<string, string>)[status ?? ''] ?? (status ?? 'Não definido');
  }
  formatDate(d?: string) { return d ? new Date(d).toLocaleDateString('pt-BR', { day: '2-digit', month: '2-digit', year: '2-digit' }) : ''; }
}
