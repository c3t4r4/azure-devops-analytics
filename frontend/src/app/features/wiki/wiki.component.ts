...
interface WikiWithCtx extends AzureWiki { orgName: string; projName: string; }
      ...
      @if (loading()) {
        ...
        <div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          @for (wiki of wikis(); track wiki.id) {
            <div class="rounded-lg border border-border bg-card p-5 hover:shadow-md transition-shadow">
              <div class="flex items-start gap-3">
                <div class="w-9 h-9 rounded-md bg-muted flex items-center justify-center flex-shrink-0">
                ...
                <div class="min-w-0">
                  ...
                  <div class="mt-1 flex items-center gap-2 flex-wrap">
                    <app-badge variant="secondary">{{ wiki.type === 'projectWiki' ? 'Projeto' : 'Código' }}</app-badge>
                    <span class="text-xs text-muted-foreground">{{ wiki.projName }}</span>
                  </div>
                  @if (wiki.remoteUrl) {
                    <a [href]="wiki.remoteUrl" target="_blank" class="mt-2 text-xs text-primary hover:underline flex items-center gap-1">
                      Abrir Wiki
    ...
    this.orgService.getAll().subscribe(orgs => {
      ...
      forkJoin(active.map(org => this.azureService.getProjects(org.name).pipe(catchError(() => of([]))))).subscribe(ppa => {
        ...
        ppa.forEach((projects, i) => projects.slice(0, 5).forEach(p => {
          calls.push(this.azureService.getWikis(active[i].name, p.id).pipe(catchError(() => of([]))));
          meta.push({ orgName: active[i].name, projName: p.name });
        }));
        if (!calls.length) { this.loading.set(false); return; }
        forkJoin(calls).subscribe(results => {
          const all: WikiWithCtx[] = [];
          results.forEach((wikis, i) => wikis.forEach((w: AzureWiki) => all.push({ ...w, ...meta[i] })));
          this.wikis.set(all);
