...
interface FeedWithCtx extends AzurePackageFeed { orgName: string; }
    ...
    <div class="space-y-6 animate-fade-in">
      ...
      @if (loading()) {
        ...
        <div class="rounded-lg border border-dashed border-border bg-card p-12 text-center">
          <p class="text-sm text-muted-foreground">Nenhum feed encontrado. Verifique as permissões de Packaging (Read).</p>
        </div>
      } @else {
            ...
            <div class="rounded-lg border border-border bg-card p-5 hover:shadow-md transition-shadow">
              <div class="flex items-start gap-3">
                ...
                <div class="min-w-0">
                  ...
                  @if (feed.description) {
                    <p class="text-xs text-muted-foreground mt-0.5 line-clamp-2">{{ feed.description }}</p>
                  }
    ...
    this.orgService.getAll().subscribe(orgs => {
      ...
      active.forEach(org => {
        this.azureService.getFeeds(org.name).subscribe({
          ...
          error: () => {},
          complete: () => { if (!--pending) { this.feeds.set(all); this.loading.set(false); } },
        });
      });
    });
  }
